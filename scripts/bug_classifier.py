...
def classify_bugs(model_name: str, bug_id: int) -> None:
    ...
    for bug in bugs:
        print(f"ORIGINAL DESCRIPTION: {bug['comments'][0]['text']}")
        bug["summary"] = translate_to_english(bug["summary"])
        for comment in bug["comments"]:
            comment["text"] = translate_to_english(comment["text"])

        print(f"TRANSLATED DESCRIPTION: {bug['comments'][0]['text']}")
        print(
            f'https://bugzilla.mozilla.org/show_bug.cgi?id={bug["id"]} - {bug["summary"]} '
        )
